...
#if defined(NDEBUG)
# error "Eurekabitcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xb01cf78d5f1c083aecfc1821eb243e91a12dfe79dce5714c9693d0b09d703e87");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Eurekabitcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Eurekabitcoin Signed Message:\n";
...
{
    // Eurekabitcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 240000 blocks, which will occur approximately every 4 years
    nSubsidy >>= (nHeight / 240000); // Eurekabitcoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Eurekabitcoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Eurekabitcoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Eurekabitcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Eurekabitcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xf6dc41e2e367bcc48cc822199e1867d2bfdb113e33c32916356e52561f3c0851");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Caliente iTechs Iconic Coin.";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04786f099edbb047766e89bdc39b8aeb36faa6ed08b7a5ba73489d4b2ab3bcffa367aad9d1860266d1e48bac9d390d5c74b12887fb967b6839f67e91167ccf4320") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1516469827;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085131524;

        if (fTestNet)
        {
            block.nTime    = 1516469838;
            block.nNonce   = 385376732;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0x8b95eeca68f36c1d7c8ca008e1f220eb263bb18aaa0d7df8d99c6e8d58895b5d"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xff, 0xc3, 0xb8, 0xdd }; // Eurekabitcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EurekabitcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EurekabitcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EurekabitcoinMiner(CWallet *pwallet)
{
    printf("EurekabitcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("eurekabitcoin-miner");
    ...
    try { loop {
        ...
        printf("Running EurekabitcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EurekabitcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EurekabitcoinMiner, pwallet));
}
